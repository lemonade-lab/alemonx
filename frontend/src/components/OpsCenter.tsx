...
import { SidebarWindowActions, SidebarWindowSectionNav } from './SidebarWindow'
...
type OpsSection = 'overview' | 'events' | 'policy' | 'audit'
...
export function OpsCenter({
  ...
  const load = useCallback(async () => {
    const statusResponse = await fetch(
      `/api/v1/ops/status?root=${encodeURIComponent(root)}`
    )
    ...
    if (readinessResponse.ok)
      setReadiness((await readinessResponse.json()) as CanaryReadiness)
  }, [root])
  useEffect(() => {
    void load()
    ...
    try {
      await fetch('/api/v1/ops/policy', {
        ...
        headers: {
          ...
          'X-Operation-Reason': operationReason
        },
  ...
  const persistentActions = (
    <>
      {onBack && (
        <button
          className={
            sidebarLayout ? 'sidebar-window-action' : 'secondary-button'
          }
          onClick={onBack}
        >
          <ArrowLeft className={sidebarLayout ? 'size-4' : 'mr-1 size-3.5'} />
          返回运行
        </button>
      )}
      {opsStatus?.enabled && (
        ...
        >
          {paused ? (
            <Play className={sidebarLayout ? 'size-4' : 'mr-1 size-3.5'} />
          ) : (
            <Pause className={sidebarLayout ? 'size-4' : 'mr-1 size-3.5'} />
          )}
          {paused ? '恢复采集' : '暂停采集'}
        </button>
      )}
      {opsStatus?.enabled && (
        ...
        >
          <OctagonX className={sidebarLayout ? 'size-4' : 'mr-1 size-3.5'} />
          紧急停止
        </button>
      )}
      {opsStatus?.enabled && (
        <button
          ...
          disabled={busy}
          onClick={() => void toggleOps(false)}
        >
          <Power className={sidebarLayout ? 'size-4' : 'mr-1 size-3.5'} />
          关闭高级运维
        </button>
      )}
      {opsStatus && !opsStatus.enabled && (
        <button
          className={sidebarLayout ? 'sidebar-window-action' : 'primary-button'}
          disabled={busy}
          onClick={() => void toggleOps(true)}
        >
          <Power className={sidebarLayout ? 'size-4' : 'mr-1 size-3.5'} />
          启用高级运维
        </button>
  ...
  const showOverview = !sidebarLayout || activeSection === 'overview'
  const showEvents = !sidebarLayout || activeSection === 'events'
  const showPolicy = !sidebarLayout || activeSection === 'policy'
  const showAudit = !sidebarLayout || activeSection === 'audit'
  return (
    <section
    ...
    >
      {sidebarLayout && (
        <SidebarWindowSectionNav>
          ...
          >
            <Activity className="size-4" /> 事件与维护
          </button>
          <button
            type="button"
            aria-current={activeSection === 'policy' ? 'page' : undefined}
            onClick={() => setActiveSection('policy')}
          >
            <SlidersHorizontal className="size-4" /> 自动化策略
          </button>
          <button
            type="button"
          ...
          >
            <BellRing className="size-4" /> 告警与审计
          </button>
        </SidebarWindowSectionNav>
      )}
      {sidebarLayout ? (
      ...
      {opsStatus && !opsStatus.enabled && (
        <div className="mx-2.5 my-4 rounded-lg border border-(--theme-border-default) bg-(--theme-surface-panel) p-5 text-sm text-(--theme-text-muted)">
          <strong className="mb-2 block text-(--theme-text-strong)">
            高级运维默认关闭
          </strong>
          <p className="mb-4">{opsStatus.message}</p>
          <p className="mb-4 text-xs">
            启用后仅开始观察此项目的运行问题，不会调用 AI、修改代码、操作 PM2
            或发送外部告警。
          </p>
        </div>
      )}
            ...
            <>
              ...
              <div className="mx-2.5 mb-4 rounded-lg border border-(--theme-border-default) bg-(--theme-surface-panel) p-3 text-xs text-(--theme-text-muted)">
                <div className="mb-2 flex items-center justify-between">
                  ...
                  <span>
                    {leases.length ? `${leases.length} 个活动租约` : '暂无租约'}
                  </span>
                </div>
                <div className="ops-lease-grid grid gap-2">
                  {leases.slice(0, 4).map(lease => (
                    <div
                      className="rounded border border-(--theme-border-subtle) px-2 py-1.5"
                      key={lease.key}
                    >
                      <div className="flex items-center justify-between gap-2">
                        <span className="truncate">{lease.key}</span>
                        <Badge
                          text={`fencing ${lease.fencingToken}`}
                          tone="info"
                        />
                      </div>
                  ...
                  >
                    ...
                    <div className="mt-2 flex flex-wrap gap-1">
                      <button
                        ...
                        onClick={() =>
                          void post(`/api/v1/ops/incidents/${item.id}/analyze`)
                        }
                      >
                        分析
                      </button>
                      <button
                        className="text-button"
                        onClick={() =>
                          void post(`/api/v1/ops/incidents/${item.id}/dry-run`)
                        }
                      >
                        预览修复
                      </button>
                      <button
                        ...
                        onClick={() =>
                          void post(
                            `/api/v1/ops/incidents/${item.id}/approve-once`
                          )
                        }
                      >
                        单次批准
                      ...
                      <button
                        className="text-button"
                        onClick={() =>
                          void post(`/api/v1/ops/incidents/${item.id}/todo`)
                        }
                      >
                        加入待办
                      </button>
                      <button
                        className="text-button"
                        onClick={() =>
                          void post(`/api/v1/ops/incidents/${item.id}/silence`)
                        }
                      >
                        静默
                      </button>
                  ...
                  >
                    ...
                    <div className="mt-1.5 flex flex-wrap gap-1">
                      {item.status !== 'done' && (
                        <button
                          className="text-button"
                          onClick={() => void completeTodo(item)}
                        >
                          完成
                        </button>
                      )}
                    ...
                    <div className="mt-1.5 flex flex-wrap gap-1">
                      {item.status === 'observing' && (
                        <button
                          ...
                          onClick={() =>
                            void post(
                              `/api/v1/ops/maintenance/${item.id}/observe`
                            )
                          }
                        >
                          结束观察
                      ...
                      {item.taskId && (
                        <button
                          ...
                          onClick={() =>
                            void post(
                              `/api/v1/ops/maintenance/${item.id}/rollback`
                            )
                          }
                        >
                          回滚
                      ...
                      {item.status === 'fixing' && (
                        <button
                          ...
                          onClick={() =>
                            void post(
                              `/api/v1/ops/maintenance/${item.id}/takeover`
                            )
                          }
                        >
                          人工接管
              ...
              {readiness && (
                <div className="mb-4 rounded-md border border-(--theme-border-default) bg-(--theme-surface-muted) p-3">
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <Badge
                      text={readiness.ready ? 'ready' : 'blocked'}
