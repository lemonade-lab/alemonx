...
export function RedisSettingsPanel() {
  ...
  const preparing =
    data.phase === 'preparing-runtime' || data.mode === 'migrating'
  const failed = data.phase === 'failed' || data.mode === 'failed'
      ...
      : failed
        ...
        : preparing
          ...
          : ready
            ? 'Redis 已准备好'
            : 'Redis 需要处理'
  const description = data.disabled
    ? '机器人和插件当前不能使用 Redis。'
    : data.external
      ? '这是已有服务，ALemonX 只连接使用，不会停止或修改它。'
      : failed
        ? '当前仍可使用临时 Redis；请检查网络后点击“重试准备”。'
        : preparing
          ? '应用正在后台准备；机器人和插件可以继续使用。'
          : ready
            ? '数据保存在本机，ALemonX 启动时会自动运行。'
            : '启动服务后，机器人和插件即可使用本地数据存储。'
  const run = async (
    action: 'start' | 'stop' | 'restart' | 'retry-runtime'
  ) => {
    if (
    ...
    try {
      ...
      setMessage(errorMessage(error, 'Redis 操作未完成。'))
    }
  }
  const save = async () => {
    ...
    if (
      ...
      return
    try {
      await saveConfig({ port: nextPort, autoStart, disabled }).unwrap()
      setChanged(false)
      setMessage('配置已保存。')
    } catch (error) {
      setMessage(errorMessage(error, 'Redis 配置未保存。'))
    }
  }
  const copy = async () => {
    try {
      await navigator.clipboard.writeText(
        data.connectionUri || `redis://${data.address}`
      )
      setCopied(true)
      window.setTimeout(() => setCopied(false), 1600)
    } catch {
      setMessage('无法复制，请手动复制连接信息。')
    }
  }
  return (
    <SettingsPage
    ...
    >
      ...
      >
        <div className="settings-service-status" aria-live="polite">
          <span
            className={
              ready ? 'is-ready' : preparing ? 'is-idle' : 'is-offline'
            }
            aria-hidden="true"
          />
