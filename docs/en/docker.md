...
The application image can be released manually through the GitHub Actions **发布腾讯云 Docker 镜像** workflow. On the repository's **Actions** page, choose the workflow and select **Run workflow**, then enter the Tencent Cloud TCR/CCR registry domain, namespace, image repository, tag, login username, and password or access token. The form defaults to the current Tencent Cloud registry and also accepts a custom Tencent Cloud registry domain. The workflow masks the password in logs; however, GitHub Actions manual forms have no password field type, so it may be retained in workflow-run metadata. Use it only in trusted repositories. The workflow builds and pushes both `linux/amd64` and `linux/arm64` images.
