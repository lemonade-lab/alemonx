...
工作区目录保存模板、工具和新建机器人；工作台状态（账户、配置、SQLite、缓存、插件）在用户配置目录（Docker 部署在宿主机 `./data`）。AI 运维数据默认在 `ops.db`（SQLite）。备份这些目录即可。

在「设置 → 工作区」可以查看并打开这些目录；如果检测到模板或内置工具存在更新版本，那里也会提示"可刷新"（不会自动覆盖你的修改）。
