...
工作台内置一个纯 Go Redis（`miniredis`），供没有独立 Redis 条件的机器人应用在本地调试时使用。默认会在工作台启动时检查配置端口：已发现 Redis 就直接复用；未发现 Redis 则自动启动内置服务。在「设置 → Redis」中可以查看状态、启动/停止服务、修改端口，或关闭自动启动。

内置 Redis 的常用数据类型会每秒保存到工作台本地目录的 `alx-redis-data.json` 快照，并在下次启动内置服务时恢复；停止、重启或正常退出时也会主动保存。外部 Redis 不受工作台控制，也不会读取或写入这个快照。
...
```bash
alx --redis-port 6380     # 调整内置 Redis 端口（会保存到配置）
alx --redis-off           # 禁止启动内置 Redis（设置页可重新启用）
```
